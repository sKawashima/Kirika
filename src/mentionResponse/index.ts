...
export const mentionResponse = async ({
  ...
  if (urls.length > 0) {
    ...
    if (urlInReplies.length === 0) {
      ...
      say({ text: modifiedMessage.endsWith(':desu:') || modifiedMessage.endsWith(':de-su:') || modifiedMessage.endsWith(':desu:。') || modifiedMessage.endsWith(':de-su:。') ? modifiedMessage : modifiedMessage.endsWith('。') ? modifiedMessage.slice(0, -1) + desuReplacement() : modifiedMessage + desuReplacement(), thread_ts });
      return
    }
